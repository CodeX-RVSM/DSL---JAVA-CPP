class HashTable:
    ...
    def linear_probing_insert(self, data, table):
        ...
        while table[index] != -1:
            ...
            if index == original_index:
                print("Table is full, cannot insert more elements")
                return

        table[index] = data
        print(f"{data} inserted in the table")

    def linear_probing_search(self, key, table):
        index = key % len(table)
        original_index = index
        comparison = 1
...
while True:
            ...
            if operation_choice == 1:
                ...
                data = (phone_no, name)
                Hash_Table.linear_probing_insert(data, Hash_Table.table)
                
                
            elif operation_choice == 2:
                 mobile_no = int(input("Enter the number you want to search: "))
                 Hash_Table.linear_probing_search(mobile_no, Hash_Table.table)
